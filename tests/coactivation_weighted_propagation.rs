...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    let outcome = (|| -> Result<(), Box<dyn std::error::Error>> {
        ...
        let outbound_hot =
            current_saliency("note-out-hot", &prefix)?.ok_or_else(missing_state_error)?;
        let outbound_cold =
            current_saliency("note-out-cold", &prefix)?.ok_or_else(missing_state_error)?;
        let inbound = current_saliency("note-inbound", &prefix)?.ok_or_else(missing_state_error)?;
