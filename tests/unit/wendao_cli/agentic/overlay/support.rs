...
pub(crate) fn write_agentic_config(
    config_path: &Path,
    prefix: &str,
...
pub(crate) fn assert_promoted_overlay_applied(
    payload: &Value,
) -> Result<(), Box<dyn std::error::Error>> {
    let overlay = payload
...
pub(crate) fn assert_verbose_overlay(payload: &Value) -> Result<(), Box<dyn std::error::Error>> {
