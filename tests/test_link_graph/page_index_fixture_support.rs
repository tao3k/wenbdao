...
use xiuxian_wendao::link_graph::{LinkGraphSemanticDocument, LinkGraphSemanticDocumentKind};
use xiuxian_wendao::{LinkGraphIndex, PageIndexNode};
...
impl PageIndexFixture {
    pub(super) fn build(scenario: &str) -> Result<Self, Box<dyn std::error::Error>> {
        let temp_dir =
            materialize_link_graph_fixture(&format!("link_graph/page_index/{scenario}/input"))?;
...
pub(super) fn read_page_index_fixture(scenario: &str, relative: &str) -> String {
    read_fixture(&format!("link_graph/page_index/{scenario}"), relative)
}

pub(super) fn assert_page_index_fixture(scenario: &str, relative: &str, actual: &Value) {
    assert_json_fixture_eq(
        &format!("link_graph/page_index/{scenario}/expected"),
        relative,
        actual,
    );
}
