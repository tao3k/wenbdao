...
use crate::link_graph::models::{
    LinkGraphMatchStrategy, LinkGraphSearchOptions, LinkGraphSemanticDocumentScope,
    LinkGraphTagFilter,
};
...
fn merge_case_and_sort(base: &mut LinkGraphSearchOptions, state: &ParsedDirectiveState) {
    ...
    if base.sort_terms.is_empty() {
        base.sort_terms = vec![crate::link_graph::models::LinkGraphSortTerm::default()];
    }
}
...
fn merge_semantic_policy(base: &mut LinkGraphSearchOptions, state: &ParsedDirectiveState) {
    if matches!(
        base.semantic_policy.document_scope,
        LinkGraphSemanticDocumentScope::All
    ) && let Some(scope) = state.semantic_document_scope
    {
        base.semantic_policy.document_scope = scope;
    }
    if base.semantic_policy.min_vector_score.is_none() {
        base.semantic_policy.min_vector_score = state.semantic_min_vector_score;
    }
    base.semantic_policy = base.semantic_policy.normalized();
}
...
pub(super) fn merge_into_base(
    ...
    merge_semantic_policy(base, state);
