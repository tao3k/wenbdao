...
impl LinkGraphIndex {
    ...
    pub(super) fn allows_semantic_edges(filters: &LinkGraphSearchFilters) -> bool {
        filters.edge_types.is_empty()
            || filters.edge_types.contains(&LinkGraphEdgeType::Semantic)
            || filters.edge_types.contains(&LinkGraphEdgeType::Verified)
            || filters.edge_types.contains(&LinkGraphEdgeType::Provisional)
    }
